...
| 3 | Slide 5's example block is real (hex `8a28308284affff`) but the card doesn't say so — "One block, 8 visits" reads as an illustrative example. That's *more* verifiable than it claims to be. | 1 (primary-source) | LOW | **Resolved 2026-09-11** — slide 5 rebuilt as the one-block timeline: the block is named on the card (Van Ness Ave & Market St, Tenderloin) and derived in `sources.md#slide-5`. |
| 4 | Slide 5 right label says "the Mission's rate would go here" but the count shown (34,714) is Mission's — correct — while the left example is a SoMa-adjacent block. Pairing an SoMa example with a Mission count on one slide invites a same-neighborhood misread. | 2 (population integrity) | MEDIUM | **Resolved 2026-09-11** — both records on slide 5 are now the SAME block (311 cases inside the hex polygon vs staff visits to the hex), on one time axis. |
| 5 | Slide 8 bullet 4 crams six agencies into one item ("For DPH/RRT, DPW… — continuous insight…"); reads as a run-on vs the crisp three above it. | 8 (cognitive load) | LOW | **Resolved 2026-09-11** — the close is four slides; the six agencies are tiles on the last one. |
